...
class _ControlSheetsGeneratorSheetState
  ...
  String _qrResultsUrl({required String sectionId, required String breed}) {
    final query = Uri(
      queryParameters: {
  ...
  Future<Map<String, Map<String, int>>> _loadCavySopSortMap() async {
    final rows = await supabase
        ...
        .select(
          'breed_name, variety_name, breed_sort_order, variety_sort_order',
        );
  ...
  Future<List<Map<String, dynamic>>> _fetchEntries() async {
    ...
    final sortedIdsToFetch = [...idsToFetch]
      ..sort((a, b) {
        final sortCmp = sortOrderForSectionId(
          a,
        ).compareTo(sortOrderForSectionId(b));
        if (sortCmp != 0) return sortCmp;

        final letterCmp = letterForSectionId(
          a,
        ).compareTo(letterForSectionId(b));
        if (letterCmp != 0) return letterCmp;

        final kindCmp = kindRankForSectionId(
          a,
        ).compareTo(kindRankForSectionId(b));
    ...
    for (final sectionId in sortedIdsToFetch) {
      ...
      for (final row in List<Map<String, dynamic>>.from(missingFurRows)) {
        ...
        final displayName = (exhibitorMap['display_name'] ?? '')
            .toString()
            .trim();
        final showingName = (exhibitorMap['showing_name'] ?? '')
            .toString()
            .trim();
        ...
        final sectionLetter = (sectionMap['letter'] ?? '')
            .toString()
            .trim()
            .toUpperCase();
        final sectionDisplayName = (sectionMap['display_name'] ?? '')
            .toString()
            .trim();
  ...
  pw.Document _buildPdf(
      ...
      final keys = grouped.keys.toList()
        ..sort((a, b) {
          ...
          if (!aIsFurOrWool && !bIsFurOrWool) {
            ...
            final classCmp = _classSortRankForPrint(
              aClass,
          ...
          final furClassCmp = _classSortRankForPrint(
            aClass,
          ).compareTo(_classSortRankForPrint(bClass));
    ...
    String sectionKindForPage(Map<String, dynamic> page) {
      final rawKind = (page['sectionKind'] ?? '')
          .toString()
          .trim()
          .toLowerCase();
      if (rawKind == 'open' || rawKind == 'youth') return rawKind;

      final title = (page['sectionTitle'] ?? '')
          .toString()
          .trim()
          .toLowerCase();
      if (title.startsWith('open') || title.contains(' open ')) return 'open';
      if (title.startsWith('youth') || title.contains(' youth ')) {
        return 'youth';
      }
    ...
    String sectionLetterForPage(Map<String, dynamic> page) {
      final rawLetter = (page['sectionLetter'] ?? '')
          .toString()
          .trim()
          .toUpperCase();
    ...
    int compareControlPages(Map<String, dynamic> a, Map<String, dynamic> b) {
      if (widget.combineSections) {
        ...
        final breedCmp = (a['breed'] ?? '').toString().toLowerCase().compareTo(
        ...
        final sectionSortCmp = pageInt(
          a,
          'sectionSortOrder',
        ).compareTo(pageInt(b, 'sectionSortOrder'));
        if (sectionSortCmp != 0) return sectionSortCmp;

        final sectionLetterCmp = sectionLetterForPage(
          a,
        ).compareTo(sectionLetterForPage(b));
        if (sectionLetterCmp != 0) return sectionLetterCmp;

        final titleCmp = (a['sectionTitle'] ?? '').toString().compareTo(
          (b['sectionTitle'] ?? '').toString(),
        );
        ...
        final groupCmp = pageInt(
          a,
          'groupSortOrder',
        ).compareTo(pageInt(b, 'groupSortOrder'));
        if (groupCmp != 0) return groupCmp;

        final varietyCmp = pageInt(
          a,
          'varietySortOrder',
        ).compareTo(pageInt(b, 'varietySortOrder'));
        if (varietyCmp != 0) return varietyCmp;

        final colorCmp = (a['color'] ?? '').toString().toLowerCase().compareTo(
          (b['color'] ?? '').toString().toLowerCase(),
        );
        if (colorCmp != 0) return colorCmp;

        final classCmp = pageInt(
          a,
          'classSortRank',
        ).compareTo(pageInt(b, 'classSortRank'));
        if (classCmp != 0) return classCmp;
      ...
      if (!widget.combineSections) {
        ...
        return (a['sectionTitle'] ?? '').toString().compareTo(
          (b['sectionTitle'] ?? '').toString(),
        );
      }
            ...
            pw.Row(
              ...
              children: [
                pw.Expanded(
                  child: pw.Row(
                    children: [
                      pw.Text('Class: $cls', style: labelStyle),
                      if (hasSex) ...[
                        pw.SizedBox(width: 24),
                        pw.Text('Sex: $sex', style: labelStyle),
                      ],
                    ],
                  ),
                ),
                ...
                pw.Column(
                  ...
                  children: [
                    pw.Text(classTotalText, style: totalStyle),
                    if (hasSex) ...[
                      pw.SizedBox(height: 2),
                      pw.Text(sexTotalText, style: totalStyle),
                    ],
                  ],
                ),
              ],
    ...
    pw.Widget compactJudgingTable({
      ...
      final h = pw.TextStyle(
        fontSize: _scaled(13),
        fontWeight: pw.FontWeight.bold,
      );
      final c = pw.TextStyle(
        fontSize: _scaled(12.5),
        fontWeight: pw.FontWeight.bold,
      );
      ...
      final sortedGroupEntries = [...groupEntries]
        ..sort((a, b) {
          final coopCompare = _coopNumberSortValue(
            _safe(a, 'coop_number'),
          ).compareTo(_coopNumberSortValue(_safe(b, 'coop_number')));
    ...
    pw.Widget qrResultsBlock({
      ...
      final url = _qrResultsUrl(sectionId: sectionId, breed: breed);
      ...
      doc.addPage(
        pw.MultiPage(
          ...
          build: (_) {
            ...
            final sexStats = <String, Map<String, int>>{};

            for (final breedName in breedNames) {
              final breedPagesForStats =
                  breedGroups[breedName] ?? const <Map<String, dynamic>>[];
              ...
              final classEntryIdsByLabel = <String, Set<String>>{};
              final classExhibitorIdsByLabel = <String, Set<String>>{};
              final sexEntryIdsByLabel = <String, Set<String>>{};
              final sexExhibitorIdsByLabel = <String, Set<String>>{};
              ...
              for (final p in breedPagesForStats) {
                ...
                final classEntryIds = classEntryIdsByLabel.putIfAbsent(
                  classKey,
                  () => <String>{},
                );
                final classExhibitorIds = classExhibitorIdsByLabel.putIfAbsent(
                  classKey,
                  () => <String>{},
                );
                final sexEntryIds = sexEntryIdsByLabel.putIfAbsent(
                  sexKey,
                  () => <String>{},
                );
                final sexExhibitorIds = sexExhibitorIdsByLabel.putIfAbsent(
                  sexKey,
                  () => <String>{},
                );
                ...
                for (
                  ...
                  classEntryIds.add(entryId);
                  sexEntryIds.add(entryId);

                  if (exhibitorId.isNotEmpty) {
                    breedExhibitorIds.add(exhibitorId);
                    groupExhibitorIds.add(exhibitorId);
                    classExhibitorIds.add(exhibitorId);
                    sexExhibitorIds.add(exhibitorId);
                  }
                }
              }
              ...
              for (final groupLabel in groupEntryIdsByLabel.keys) {
                groupStats['$breedName|$groupLabel'] = {
                  ...
                  'exhibitors':
                      groupExhibitorIdsByLabel[groupLabel]?.length ?? 0,
                };
              }

              for (final classKey in classEntryIdsByLabel.keys) {
                classStats['$breedName|$classKey'] = {
                  'entries': classEntryIdsByLabel[classKey]?.length ?? 0,
                  'exhibitors': classExhibitorIdsByLabel[classKey]?.length ?? 0,
                };
              }

              for (final sexKey in sexEntryIdsByLabel.keys) {
                sexStats['$breedName|$sexKey'] = {
                  'entries': sexEntryIdsByLabel[sexKey]?.length ?? 0,
                  'exhibitors': sexExhibitorIdsByLabel[sexKey]?.length ?? 0,
                };
              }
            }
            ...
            for (
              var breedIndex = 0;
              breedIndex < breedNames.length;
              breedIndex++
            ) {
              final breed = breedNames[breedIndex];
              final breedPages =
                  breedGroups[breed] ?? const <Map<String, dynamic>>[];
              ...
              for (var i = 0; i < breedPages.length; i++) {
                ...
                final classBlockWidgets = <pw.Widget>[
                  compactClassHeaderBlock(
                    ...
                    groupCount: groupStats[groupStatsKey]?['entries'] ?? 0,
                    groupExhibitorCount:
                        groupStats[groupStatsKey]?['exhibitors'] ?? 0,
                    classCount: classStats[classStatsKey]?['entries'] ?? 0,
                    classExhibitorCount:
                        classStats[classStatsKey]?['exhibitors'] ?? 0,
                ...
                classBlockWidgets.add(
                  compactJudgingTable(
                    groupEntries: (p['rows'] as List)
                        .cast<Map<String, dynamic>>(),
                    specialsList: (p['specials'] as List)
                        .map((x) => x.toString())
                        .toList(),
  ...
  pw.Document _buildBlankPdf(pw.ThemeData theme) {
    ...
    String sectionTitle(String sectionId) {
      ...
      return [kind, letter].where((part) => part.isNotEmpty).join(' ');
    }

    final titles = sectionIds
        .map(sectionTitle)
        .where((title) => title.isNotEmpty)
        .toList();
    if (titles.isEmpty) titles.add(widget.sectionLabel);

    pw.Widget line() => pw.Expanded(
      child: pw.Container(
        height: 11,
  ...
  Future<void> _generateBlankPdf() async {
    ...
    try {
      final theme = await buildPrintPackPdfTheme();
      final bytes = await _buildBlankPdf(theme).save();
      final savedPath = await savePdfToUserChosenLocation(
        bytes: Uint8List.fromList(bytes),
        suggestedName:
            'blank_control_sheet_${widget.showName}_${widget.sectionLabel}.pdf',
      );
      if (!mounted) return;
      setState(() {
        _building = false;
