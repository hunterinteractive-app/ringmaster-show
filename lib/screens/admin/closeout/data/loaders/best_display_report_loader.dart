...
class BestDisplayReportLoader {
  ...
  Future<BestDisplayReportData> load(ReportRequest request) async {
    ...
    if (showId.isEmpty) {
      throw Exception('BestDisplayReportLoader requires request.showId.');
    }
    ...
    final rows = _parseStandingRows(rawStandings)
        .where(
          (row) => species.isEmpty || _normalizeSpecies(row.species) == species,
        )
        .toList();
    ...
    return BestDisplayReportData(
      showId: showId,
      showName: _string(showRow['name'], fallback: 'Unnamed Show'),
      showDate: _formatDateRange(showRow['start_date'], showRow['end_date']),
  ...
  List<BestDisplayStandingRow> _parseStandingRows(Object? rawValue) {
    ...
    return rawValue
        .map<BestDisplayStandingRow>((rawRow) {
          if (rawRow is! Map) {
            throw Exception(
  ...
  List<BestDisplaySectionData> _groupSections(
    ...
    for (final row in rows) {
      ...
      grouped.putIfAbsent(key, () => <BestDisplayStandingRow>[]);
      grouped[key]!.add(row);
    }
  ...
  List<BestDisplayBreedSectionData> _buildBreedSections(
    ...
    for (final rawRow in rawValue) {
      ...
      final breedName = _string(row['breed_name'], fallback: 'Unknown Breed');
      ...
      if (requestedSpecies.isNotEmpty &&
          _normalizeSpecies(species) != requestedSpecies) {
        continue;
      }

      final key = '$sectionId|$species|${breedName.toLowerCase()}|$exhibitorId';
    ...
    for (final group in grouped.values) {
      group.sort((a, b) {
        final eligibleA = a.qualifyingEntryCount >= minimumEntriesRequired;
        final eligibleB = b.qualifyingEntryCount >= minimumEntriesRequired;
      ...
      final eligiblePointTotals =
          group
              .where(
                (row) => row.qualifyingEntryCount >= minimumEntriesRequired,
              )
      ...
      final rows = group
          .map((aggregate) {
            ...
            final tiedCount = isEligible
                ? group.where((row) {
                    return row.qualifyingEntryCount >= minimumEntriesRequired &&
    ...
    result.sort((a, b) {
      ...
      final speciesCompare = _speciesRank(
        a.species,
      ).compareTo(_speciesRank(b.species));
      if (speciesCompare != 0) return speciesCompare;

      return a.breedName.toLowerCase().compareTo(b.breedName.toLowerCase());
    });

    return List<BestDisplayBreedSectionData>.unmodifiable(result);
  }

  int _compareSections(BestDisplaySectionData a, BestDisplaySectionData b) {
    final scopeCompare = _scopeRank(a.scope).compareTo(_scopeRank(b.scope));
    if (scopeCompare != 0) return scopeCompare;
  ...
  int _compareStandingRows(BestDisplayStandingRow a, BestDisplayStandingRow b) {
    ...
    final eligibleRankCompare = (a.eligibleRank ?? 999999).compareTo(
      b.eligibleRank ?? 999999,
    );
