...
List<ResultsEntryBlockingIssue> validateRabbitResults({
  ...
  void validateBuckets({
    ...
    String Function(Map<String, dynamic>)? issueGroupScope,
  }) {
        ...
        if (winners.length > 1) {
          issues.add(
            ResultsEntryBlockingIssue(
              ...
              message:
                  '$award is assigned to more than one rabbit in this $scopeLabel: ${entryLabel(winners[0])} and ${entryLabel(winners[1])}.',
              entry: winners[0],
              conflictsWith: winners[1],
              level: level,
              awardCode: award,
              groupScope: issueGroupScope?.call(winners[0]),
            ),
          );
        }
      }
      ...
      if (primaryWinner == null) {
        issues.add(
          ResultsEntryBlockingIssue(
            ...
            message:
                'Select one eligible $primary winner for this rabbit $scopeLabel.',
            entry: eligible.first,
            level: level,
            awardCode: primary,
            groupScope: issueGroupScope?.call(eligible.first),
          ),
        );
        continue;
      }
      ...
      if (oppositeCandidates.isNotEmpty && oppositeWinner == null) {
        issues.add(
          ResultsEntryBlockingIssue(
            ...
            message:
                'Select the eligible opposite-sex $opposite winner for this rabbit $scopeLabel.',
            entry: oppositeCandidates.first,
            level: level,
            awardCode: opposite,
            groupScope: issueGroupScope?.call(oppositeCandidates.first),
          ),
        );
      }
    }
  ...
  if (requireVarietyAwards) {
    validateBuckets(
      values: buckets((entry) {
        final scope = [sectionId(entry), breed(entry), variety(entry)];
  ...
  if (requireGroupAwards) {
    validateBuckets(
      ...
      scopeLabel: 'rabbit group',
      level: ResultsValidationIssueLevel.group,
      issueGroupScope: group,
    );
  }
