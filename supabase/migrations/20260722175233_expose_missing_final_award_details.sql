...
eligible_rows as (
  select
    ...
    lower(trim(coalesce(nullif(variety, ''), variety_name, ''))) as variety_key,
...
required_entries as (
  select distinct entry_id, section_id
...
normalized_final_awards as (
  select distinct
    ...
    case
      ...
      when regexp_replace(lower(coalesce(ea.award_code, '')), '[^a-z0-9]+', '', 'g')
        in ('1ris', '1stris', 'firstris', '1streserveinshow',
            'firstreserveinshow') then '1RIS'
      when regexp_replace(lower(coalesce(ea.award_code, '')), '[^a-z0-9]+', '', 'g')
        in ('2ris', '2ndris', 'secondris', '2ndreserveinshow',
            'secondreserveinshow') then '2RIS'
      else null
    end as award_kind
  from required_entries re
...
required_final_award_types as (
  ...
  from (values
    ('BIS', 'Best in Show', 1),
    ('1RIS', 'First Reserve in Show', 2),
    ('2RIS', 'Second Reserve in Show', 3)
  ) as awards(award_kind, award_label, award_order)
),
final_award_counts as (
  ...
  left join final_awards fa
    on fa.section_id = s.section_id and fa.award_kind = t.award_kind
  group by s.section_id, s.section_label, s.sort_order,
    t.award_kind, t.award_label, t.award_order
),
missing_final_awards as (
  select count(*) filter (where winner_count < 1)::int as total
...
missing_final_award_details as (
  select coalesce(
    jsonb_agg(
      ...
      ) order by sort_order, section_label, award_order
    ) filter (where winner_count < 1),
    '[]'::jsonb
  ) as items
  from final_award_counts
),
...
same_entry_final_award_conflicts as (
  select count(*)::int as total
  from (
    select section_id, entry_id
    from final_awards
    group by section_id, entry_id
...
counts as (
  select
    ...
    ((select total from duplicate_final_award_winners) +
      (select total from same_entry_final_award_conflicts))::int
      as duplicate_final_award_count,
...
) rename to get_closeout_dashboard_scoped_without_readiness_details;
...
as $function$
select jsonb_set(
  public.get_closeout_dashboard_scoped_without_readiness_details(
    p_show_id,
    p_scope_key,
    p_section_ids,
    p_artifact_limit,
...
revoke all on function public.get_closeout_dashboard_scoped_without_readiness_details(
  uuid, text, uuid[], integer, integer, public.report_type
) from public, anon;
grant execute on function public.get_closeout_dashboard_scoped_without_readiness_details(
  uuid, text, uuid[], integer, integer, public.report_type
) to authenticated, service_role;
