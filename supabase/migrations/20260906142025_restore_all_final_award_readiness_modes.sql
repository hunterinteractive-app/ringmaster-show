...
comment on function public.show_results_readiness_scoped(uuid, uuid[]) is
  'Checks scoped result and final-award readiness for four/six/BIS, BIS/RIS, and BIS/1RIS/2RIS modes; placement 0 is excluded from duplicate-rank detection.';
...
begin
  ...
  if strpos(v_definition, $$ss.final_award_mode = 'four_six_bis'$$) = 0
    or strpos(v_definition, $$ss.final_award_mode = 'bis_ris'$$) = 0
    or strpos(v_definition, $$ss.final_award_mode = 'bis_1ris_2ris'$$) = 0
    or strpos(v_definition, $$placement <> '0'$$) = 0 then
    raise exception 'Canonical final-award readiness validator was not installed';
  end if;
end;
$verify$;
