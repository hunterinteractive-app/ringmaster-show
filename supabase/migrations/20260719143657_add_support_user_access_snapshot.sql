...
comment on function public.support_user_access_snapshot(uuid) is
  'Returns the access signals Support mode needs to mirror a target user. Callable only by a signed-in super administrator.';

revoke all on function public.support_user_access_snapshot(uuid) from public;
revoke all on function public.support_user_access_snapshot(uuid) from anon;
grant execute on function public.support_user_access_snapshot(uuid) to authenticated;
grant execute on function public.support_user_access_snapshot(uuid) to service_role;
