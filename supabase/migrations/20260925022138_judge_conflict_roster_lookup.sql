-- Include the actual show roster and judges already in the lineup.
...
begin
  ...
  with judge_people as (
    ...
    where j.id in (
      select ja.judge_id from public.judge_assignments ja where ja.show_id=p_show_id
      union
      select sj.judge_id from public.show_judges sj where sj.show_id=p_show_id
      union
      select a.judge_id from public.show_judging_assignments a
      where a.show_id=p_show_id and a.judge_id is not null
    )
  ), own_exhibitors as (
