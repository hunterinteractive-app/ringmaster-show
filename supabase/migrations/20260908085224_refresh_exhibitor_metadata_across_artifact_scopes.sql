-- A finalize run has one run-wide scope key, while each exhibitor artifact has
-- its own canonical scope key.  Requeue All passes the run-wide key, so
-- requiring every artifact's scope_key to equal p_scope_key prevents any
-- exhibitor metadata from refreshing.  Single-artifact requeues still bind to
-- the requested artifact id and scope; whole-run requeues refresh every current
-- exhibitor artifact in the requested show/finalize run.
  ...
  where a.show_id = p_show_id
    ...
    and (
      (
        ...
        -- p_scope_key is the finalize-run scope during Requeue All, not an
        -- individual artifact scope.
        and exists (
          select 1
...
comment on function public.refresh_closeout_exhibitor_artifact_metadata(
  uuid, uuid, text, uuid
) is 'Refreshes current exhibitor report metadata for either one canonical artifact scope or every exhibitor artifact in a validated finalize-run scope.';
