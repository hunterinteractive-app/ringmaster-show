-- Dashboard polls can cache plans while the report tables are empty. First
-- finalization then inserts thousands of rows before statistics are refreshed;
-- the cached one-row estimates cause repeated full queue/materialized-CTE scans.
-- Use PL/pgSQL so the base query participates in custom planning, and scope
-- custom plans to the dashboard call chain. Query contents, permissions,
-- pagination, and the caller's statement timeout are unchanged.

CREATE OR REPLACE FUNCTION public.get_closeout_dashboard_scoped_without_activity(p_show_id uuid, p_scope_key text, p_section_ids uuid[], p_artifact_limit integer DEFAULT 100, p_artifact_offset integer DEFAULT 0, p_report_name public.report_type DEFAULT NULL::public.report_type)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE
 SET search_path TO ''
...
-- This setting also reaches the nested PL/pgSQL activity/review/count queries.
-- It is restored automatically when the dashboard function returns.
