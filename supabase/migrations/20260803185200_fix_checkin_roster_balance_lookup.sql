...
begin
  ...
  left join lateral (
    select coalesce(sum(show_balance.balance_due_cents), 0)::integer as total_due_cents
    from public.show_exhibitor_balances as show_balance
    where show_balance.show_id = p_show_id
      and show_balance.exhibitor_id = e.exhibitor_id
  ) as balance_summary on true
  ...
  group by e.exhibitor_id, x.display_name, x.first_name, x.last_name, x.exhibitor_number, r.status, r.completed_at, balance_summary.total_due_cents
