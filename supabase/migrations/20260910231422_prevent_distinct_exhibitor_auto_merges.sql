-- Preserve capital initials when comparing names, and never automatically
-- merge exhibitors with different types or conflicting known birth dates.
...
declare
  ...
  reason constant text :=
    'Automated monthly deduplication: exact normalized name, email, and phone; same exhibitor type and no conflicting birth date, account, or ARBA number.';
begin
  if p_apply then
  ...
  for candidate_group in
    ...
    join eligible_groups g using (name_key, email_key, phone_key)
