-- Read-only data for one verified exhibitor. The opaque session token is
-- hashed before lookup and never persisted or returned again.
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'show_label', coalesce(nullif(sec.display_name, ''), initcap(coalesce(sec.kind, 'Show')) || ' ' || upper(coalesce(sec.letter, ''))),
    'species', e.species,
    'tattoo', e.tattoo,
