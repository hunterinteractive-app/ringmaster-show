...
required_final_award_types as (
  select unnest(array['BIS', '1RIS', '2RIS']) as award_kind
),
final_award_counts as (
  select s.section_id, t.award_kind,
  ...
  group by s.section_id, t.award_kind
),
...
begin
  ...
  if v_definition is null then
    raise exception 'public.finalize_show_scoped(uuid, uuid[], text, text) is required';
  end if;

  if strpos(v_definition, 'public.show_results_readiness(p_show_id)') = 0 then
    raise exception 'Expected whole-show readiness call was not found';
  end if;

  v_updated := replace(
    v_definition,
    'public.show_results_readiness(p_show_id)',
    'public.show_results_readiness_scoped(p_show_id, v_section_ids)'
  );
  execute v_updated;
end;
$block$;
